...
export const handleReactionButtonInteraction = (client: Client) => {
    client.on('interactionCreate', async (interaction: Interaction) => {
        ...
        try {
            const reactionRole = await ReactionRole.findOne({ messageId: buttonInteraction.message.id });
            if (!reactionRole) {
                await buttonInteraction.reply({ content: '⚠️ Keine Reaktionsrollen für diese Nachricht gefunden.', ephemeral: true });
                return;
            }

            const roleData = reactionRole.roles.find(r => r.emoji === emoji);
            if (!roleData) {
                await buttonInteraction.reply({ content: `⚠️ Keine Rolle für Emoji gefunden: ${emoji}`, ephemeral: true });
                return;
            }
            ...
            await buttonInteraction.reply({ content: '⚠️ Es gab einen Fehler beim Hinzufügen oder Entfernen der Rolle. Bitte versuche es später erneut.', ephemeral: true });
        }
    });
};
