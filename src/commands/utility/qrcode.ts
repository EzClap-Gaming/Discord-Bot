...
const QRCodeCommand: Command = {
    ...
    async execute(interaction: ChatInputCommandInteraction) {
        ...
        try {
            ...
            await interaction.reply({ content: '⚠️ Bitte gib eine gültige URL an.', ephemeral: true });
            return;
        }

        try {
            const qrBuffer = await QRCode.toBuffer(url, { type: 'png', errorCorrectionLevel: 'H' });
            ...
            await interaction.reply({ content: '⚠️ Es ist ein Fehler beim Generieren des QR-Codes aufgetreten.', ephemeral: true });
        }
    }
};
