    ...
    async execute(interaction: ChatInputCommandInteraction) {
        if (!interaction.guild) {
            await interaction.reply({
                content: "⚠️ Dieser Befehl kann nur auf einem Server verwendet werden.",
                ephemeral: true,
            });
            return;
        }
        ...
        try {
            ...
            await interaction.reply({
                content: "⚠️ Es gab einen Fehler beim Abrufen der Serverstatistiken.",
                ephemeral: true,
            });
        }
    },
