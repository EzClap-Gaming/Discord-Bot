...
const PDFCommand: Command = {
    data: new SlashCommandBuilder()
        ...
        .addStringOption(option =>
            option.setName('title')
                .setDescription('Titel des PDF-Dokuments')
                .setRequired(true))
        .addStringOption(option =>
            option.setName('text')
    ...
    async execute(interaction: ChatInputCommandInteraction) {
        ...
        try {
            ...
            const attachment = new AttachmentBuilder(pdfBuffer, { name: 'dokument.pdf' });
            ...
            await interaction.reply({ content: '⚠️ Es ist ein Fehler beim Erstellen der PDF aufgetreten.', ephemeral: true });
        }
    }
};
