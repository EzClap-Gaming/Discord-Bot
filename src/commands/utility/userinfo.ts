...
const UserInfoCommand: Command = {
    data: new SlashCommandBuilder()
        ...
        .addUserOption(option => option.setName('user').setDescription('Der Benutzer, über den Informationen angezeigt werden').setRequired(false)),
    async execute (interaction: ChatInputCommandInteraction) {
        ...
        const embed = new EmbedBuilder()
            ...
            .addFields(
                ...
                { name: 'Beigetreten', value: member ? `<t:${Math.floor(member.joinedTimestamp! / 1000)}:F>` : `🚫 Nicht Mitglied des Servers`, inline: true },
                { name: 'Konto erstellt', value: `<t:${Math.floor(user.createdTimestamp / 1000)}:F>`, inline: true },
            )
