...
const BotInfoCommand: Command = {
    data: new SlashCommandBuilder()
        .setName('botinfo')
        .setDescription('Zeigt Informationen über den Bot an. Dies kann nur vom Besitzer genutzt werden.'),
    ...
    async execute(interaction: ChatInputCommandInteraction) {
        try {
            ...
            const botInfoEmbed = new EmbedBuilder()
                ...
                .addFields(
                    { name: 'Bot-Name', value: `${interaction.client.user?.username || 'N/A'} 🤖`, inline: true },
                    { name: 'Server', value: `${interaction.client.guilds.cache.size} 🌍`, inline: true },
                    { name: 'Benutzer', value: `${interaction.client.users.cache.size} 👥`, inline: true },
                    { name: 'Betriebszeit', value: uptime, inline: true },
                    { name: 'Speicherverbrauch', value: `${memoryUsage} MB`, inline: true },
                    { name: 'Bot-Version', value: botVersion, inline: true }
                )
                .setFooter({ text: 'Bot-Info ⚡', iconURL: interaction.client.user?.displayAvatarURL() })
                .setTimestamp();

            await interaction.reply({ embeds: [botInfoEmbed], ephemeral: true });
        } catch (error) {
            console.error('Error retrieving bot info:', error);
            await interaction.reply({ content: 'Es ist ein Fehler beim Abrufen der Bot-Informationen aufgetreten. Bitte versuche es später noch einmal. ⚠️', ephemeral: true });
        }
    },
};
