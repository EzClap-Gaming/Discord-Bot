    ...
    async execute(interaction: ChatInputCommandInteraction) {
        ...
        if (commandName === 'level') {
            ...
            if (subcommand === 'fortschritt') {
                ...
                const embed = new EmbedBuilder()
                    ...
                    .addFields(
                        ...
                        { name: 'XP bis zum nächsten Level', value: `${xpNeeded}`, inline: true }
                    )
                    .setImage(chartUrl)
                    .setFooter({ text: `Angefordert von ${interaction.user.username}`, iconURL: interaction.user.displayAvatarURL() });
            ...
            if (subcommand === 'rangliste') {
                ...
                const leaderboardWithAvatars = await Promise.all(leaderboard.map(async (user) => {
            ...
            if (subcommand === 'statistiken') {
                ...
                const recentXpHistory = userData.xpHistory.filter(entry => entry.date >= thirtyDaysAgo);
                if (recentXpHistory.length === 0) {
                    interaction.reply({ embeds: [new EmbedBuilder().setColor('Random').setDescription('Keine XP-Daten für die letzten 30 Tage gefunden.')] });
                    return;
                }

                const totalXp = recentXpHistory.reduce((acc, entry) => acc + entry.xpEarned, 0);
                const xpPerDay = recentXpHistory.reduce((acc, entry) => {
                ...
                const embed = new EmbedBuilder()
                    ...
                    .addFields(
                        ...
                        { name: 'Gesamt XP (Letzte 30 Tage)', value: `${totalXp}`, inline: true },
                        { name: 'Gesamt Level-Ups', value: `${userData.levelUpCount || 0}`, inline: true }
                    )
                    .setImage(chartUrl)
                    .setFooter({ text: `Angefordert von ${interaction.user.username}`, iconURL: interaction.user.displayAvatarURL() });
