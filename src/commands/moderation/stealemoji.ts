    ...
    async execute(interaction: ChatInputCommandInteraction) {
        ...
        try {
            ...
            if (!botMember.permissions.has(PermissionFlagsBits.ManageGuildExpressions)) {
                await interaction.reply({ content: 'I do not have the required permissions to manage emojis.', ephemeral: true });
                return;
            }

            if (!member.permissions.has(PermissionFlagsBits.ManageGuildExpressions)) {
                await interaction.reply({ content: 'You do not have the required permissions to manage emojis.', ephemeral: true });
                return;
            }
            ...
            await interaction.reply({ content: `Emoji ${name} has been stolen and added to the server.`, ephemeral: true });
        } catch (error) {
            console.error(`[Discord] Error while stealing emoji: `, error);
            await interaction.reply({ content: 'There was an error while stealing the emoji.', ephemeral: true });
        }
    }
};
