...
const WarnCommand: Command = {
    ...
    async execute(interaction: ChatInputCommandInteraction) {
        ...
        if (subcommand === 'add') {
            ...
            try {
                const warningCount = await Warn.countDocuments({ guildId: interaction.guildId!, userId: member.id });
                ...
                const dmEmbed = new EmbedBuilder()
                    ...
                    .setFooter({ text: `Issued by ${interaction.user.username}`, iconURL: interaction.user.displayAvatarURL() })
                    .setTimestamp();
                ...
                const embed = new EmbedBuilder()
                    .setColor('#FF0000')
                    .setTitle('Error')
                    .setDescription('An error occurred while adding the warning. Please try again later.')
                    .setFooter({ text: `Executed by ${interaction.user.username}`, iconURL: interaction.user.displayAvatarURL() })
                    .setTimestamp();
                await interaction.reply({ embeds: [embed], ephemeral: true });
            }
        ...
        if (subcommand === 'remove') {
            ...
            try {
                const warnings = await Warn.find({ guildId: interaction.guildId!, userId: member.id });
                const warningToRemove = warnings.find(w => w.reason.includes(identifier));

                if (!warningToRemove) {
                    const embed = new EmbedBuilder()
                        .setColor('#FF0000')
                        .setDescription(`No warning found for ${member.user.tag} with identifier: "${identifier}".`)
                        .setFooter({ text: `Executed by ${interaction.user.username}`, iconURL: interaction.user.displayAvatarURL() });
                    await interaction.reply({ embeds: [embed], ephemeral: true });
                    return;
                }
                ...
                const embed = new EmbedBuilder()
                    ...
                    .setDescription(`The following warning has been removed from ${member.user.tag}:`)
                ...
                const dmEmbed = new EmbedBuilder()
                    ...
                    .setDescription(`A warning has been removed from your record in the server. Removed Reason: ${removalReason}\nOriginal Warning: ${warningToRemove.reason}`)
                ...
                const embed = new EmbedBuilder()
                    .setColor('#FF0000')
                    .setTitle('Error')
                    .setDescription('An error occurred while removing the warning. Please try again later.')
                    .setFooter({ text: `Executed by ${interaction.user.username}`, iconURL: interaction.user.displayAvatarURL() })
                    .setTimestamp();
                await interaction.reply({ embeds: [embed], ephemeral: true });
            }
        ...
        if (subcommand === 'list') {
            try {
                ...
                const embed = new EmbedBuilder()
                    .setColor('#FF0000')
                    .setTitle('Error')
                    .setDescription('An error occurred while listing the warnings. Please try again later.')
                    .setFooter({ text: `Executed by ${interaction.user.username}`, iconURL: interaction.user.displayAvatarURL() })
                    .setTimestamp();
                await interaction.reply({ embeds: [embed], ephemeral: true });
            }
