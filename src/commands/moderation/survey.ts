...
const SurveyCommand: Command = {
    data: new SlashCommandBuilder()
        ...
        .addStringOption((option) =>
            option.setName("option1").setDescription("Option 1 for the survey").setRequired(true),
        )
        .addStringOption((option) =>
            option.setName("option2").setDescription("Option 2 for the survey").setRequired(true),
        )
        .addStringOption((option) =>
            option.setName("option3").setDescription("Option 3 for the survey").setRequired(false),
        )
        .addStringOption((option) =>
            option.setName("option4").setDescription("Option 4 for the survey").setRequired(false),
        ),
    async execute(interaction: ChatInputCommandInteraction) {
        try {
            ...
            await interaction.reply({
                content: "An error occurred while creating the survey. Please try again later.",
                ephemeral: true,
            });
        }
    },
