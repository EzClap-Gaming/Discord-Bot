...
const KickCommand: Command = {
    ...
    async execute(interaction: ChatInputCommandInteraction) {
        ...
        if (memberToKick.roles.highest.position >= (interaction.member as GuildMember).roles.highest.position) {
            await interaction.reply({ content: `You cannot kick this member because they have a higher or equal role to yours.`, ephemeral: true });
            return;
        }

        if (!memberToKick.kickable) {
            await interaction.reply({ content: `I cannot kick this member. They might have a higher role than me or I lack permissions.`, ephemeral: true });
            return;
        }

        try {
            await memberToKick.kick(reason);
            await interaction.reply({ content: `Successfully kicked ${memberToKick.user.tag} from the server.`, ephemeral: true });
        } catch (error) {
            console.error(`[DISCORD] Error kicking member: `, error);
            await interaction.reply({ content: 'There was an error while kicking this member.', ephemeral: true });
        }
    }
};
