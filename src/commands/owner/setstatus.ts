...
const SetStatusCommand: Command = {
    data: new SlashCommandBuilder()
        .setName("set-status")
        .setDescription(
            "Setzt die Statusnachricht des Bots. Dies kann nur vom Bot-Besitzer verwendet werden. 🛠️",
        )
    ...
    async execute(interaction: ChatInputCommandInteraction) {
        try {
            ...
            const errorEmbed = new EmbedBuilder()
                .setColor("Random")
                .setDescription(
                    "⚠️ Ein Fehler ist beim Aktualisieren des Bot-Status aufgetreten. Bitte versuche es später erneut.",
                );
